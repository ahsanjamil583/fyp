...
export function CustomerBusinessPage() {
  ...
  return (
    <section className="space-y-6">
      ...
      <form className="flex gap-2 rounded-xl border border-line bg-white p-5 shadow-card" onSubmit={searchItems}>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {items.map((item) => (
          <div key={item.id} className="rounded-xl border border-line bg-white p-4 shadow-card">
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button className="rounded-xl border border-line px-3 py-1.5 text-sm font-semibold text-ink" onClick={() => toggleFavorite(item.id)}>
                  {favorites.some((favorite) => favorite.item?.id === item.id && favorite.tenant?.id === business.id) ? "Saved" : "Save"}
                </button>
                <button className="rounded-xl bg-brand px-3 py-1.5 text-sm font-semibold text-white" onClick={() => addToCart(item.id)}>
                  Add to cart
                </button>
              </div>
            </div>
